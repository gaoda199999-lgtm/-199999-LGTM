# -*- coding: utf-8 -*-
"""
矿山探采对比分析报告生成工具 v12.1（性能优化 + Bug 修复 + 代码质量提升 + 数据库全局化 + 进度反馈）
─────────────────────────────────────────────────────────────
【V12.0 优化内容】
  ...
  10. 版本号更新为 v12.0。
  11. [V12.1] 数据库全局化：所有季度数据汇聚到统一目录，支持跨年度趋势分析。
  12. [V12.1] 处理进度条：读取数据和生成图表时打印进度，告别黑窗干等。
  13. [V12.1] sf() 警告机制：遇到非数值数据时发出警告，不再静默吞错。
  14. [V12.1] 配色完全可覆盖：config.json 中的 colors/bar_label_fmt 可完整替换默认值。
  15. [V12.1] PPT 结论摘要页：演示文稿首页展示核心结论，提升可汇报性。
...
DEFAULT_CONFIG = {
    ...
    # V12.1 优化：数据库全局化，所有季度数据汇聚到统一目录
    "database": {
        "base_dir": "~/Desktop/矿山探采对比数据库"
    }
}
...
def load_config(config_path):
    ...
    if os.path.exists(config_path):
        try:
            ...
            # V12.1 优化：colors 和 bar_label_fmt 支持用户完全覆盖（而非合并）
            if "colors" in user_cfg:
                cfg["colors"] = user_cfg["colors"]
            if "bar_label_fmt" in user_cfg:
                cfg["bar_label_fmt"] = user_cfg["bar_label_fmt"]
...
# V12.1 优化：sf() 增加可选警告，遇到非数值数据时提醒用户
_sf_warned_keys = set()

def sf(v, warn_key=None):
    try:
        return float(v)
    except (ValueError, TypeError):
        if warn_key and warn_key not in _sf_warned_keys:
            _sf_warned_keys.add(warn_key)
            warnings.warn(f"⚠ 数据警告：字段 [{warn_key}] 包含非数值数据 '{v}'，已替换为 0。请检查 Excel 原始数据。")
        return 0.0
...
def read_pinghua(filepath, config):
    ...
    for sn in wb.sheetnames:
        # V12.1 优化：处理进度反馈
        print(f" ⏳ 正在读取贫化损失表工作表 [{wb.sheetnames.index(sn)+1}/{len(wb.sheetnames)}]：{sn}")
        ws = wb[sn]
        rows = list(ws.iter_rows(min_row=1, values_only=True))
        h_idx = None
        for i, r in enumerate(rows):
...
def read_duibai(filepath, config):
    ...
    for sn in wb.sheetnames:
        if not any(pat in sn for pat in sheet_patterns):
            continue
        # V12.1 优化：处理进度反馈
        print(f" ⏳ 正在读取探采对比表工作表 [{wb.sheetnames.index(sn)+1}/{len(wb.sheetnames)}]：{sn}")
        ws = wb[sn]
        rows = list(ws.iter_rows(min_row=1, values_only=True))
        if not rows:
            continue
...
def generate_pptx(cfg, cfg_paths, png_files_map, conclusions_summary=""):
    prs = Presentation()
    ...
    chapters = [
        ...
        # V12.1 优化：PPT 增加结论摘要页
        {"type": "content", "title": "核心结论摘要",
         "text": conclusions_summary,
         "imgs": None},
...
def make_report(cfg_paths, cfg, ph_data, db_data):
    ...
    # ===== V10 特性：SQLite 历史数据库 =====
    # V12.1 优化：数据库全局化，所有季度数据汇聚到统一目录
    db_dir = os.path.expanduser(cfg.get("database", {}).get("base_dir", "~/Desktop/矿山探采对比数据库"))
    ...
    for rec in step_area:
        step = rec.get("_step", "")
        # V12.1 优化：sf() 增加 warn_key 参数，遇到非数值数据时提醒用户
        prod = sf(col_match.get_or(rec, "面积_生产"), warn_key=f"{step}_面积_生产")
        zhtk = sf(col_match.get_or(rec, "面积_ZHTK"), warn_key=f"{step}_面积_ZHTK")
        if not zhtk:
            continue
        abs_e = zhtk - prod
    ...
    for rec in ore_steps:
        nm = rec.get("_step", "")
        # V12.1 优化：sf() 增加 warn_key 参数，遇到非数值数据时提醒用户
        p_q = sf(col_match.get_or(rec, "生产_矿石量"), warn_key=f"{nm}_生产_矿石量")
        p_c = sf(col_match.get_or(rec, "生产_品位"), warn_key=f"{nm}_生产_品位")
        p_p = sf(col_match.get_or(rec, "生产_金属量"), warn_key=f"{nm}_生产_金属量")
        z_q = sf(col_match.get_or(rec, "ZHTK_矿石量"), warn_key=f"{nm}_ZHTK_矿石量")
        z_c = sf(col_match.get_or(rec, "ZHTK_品位"), warn_key=f"{nm}_ZHTK_品位")
        z_p = sf(col_match.get_or(rec, "ZHTK_金属量"), warn_key=f"{nm}_ZHTK_金属量")
        step_detail[nm] = {
    ...
    # V12.1 优化：PPT 增加结论摘要页，传入核心结论
    conclusions_summary = "\n".join(conclusions[:3]) if conclusions else "暂无核心结论"
    pptx_path = generate_pptx(cfg, cfg_paths, png_files_map, conclusions_summary)
...
def parse_args():
    parser = argparse.ArgumentParser(description="矿山探采对比分析报告生成工具 v12.1")
...
def main():
    ...
    print("\n" + "=" * 64)
    print(f"  {cfg['mine']['name']} 探采对比分析报告生成工具 v12.1（优化版）")
